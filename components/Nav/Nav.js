...
export const Nav = () => {
  ...
  const activeLink = "flex gap-1 mb-2 p-1 bg-white text-blue-900 rounded-l-lg";
  ...
  return (
    <aside className="text-white pt-4 pb-4 pl-4" data-testId="nav">
      ...
      {tabs.map((tab) => (
        <Tab title={tab.title} link={tab.link} tabStyle={activeTab === tab.id ? activeLink : inactiveLink} onClick={() => handleTabClick(tab.id)} />
      ))}
